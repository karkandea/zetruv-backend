...
[[ "$API_DOMAIN" == "api-dev.zetruv.com" ]] || { echo "Unexpected DEV API_DOMAIN: $API_DOMAIN" >&2; exit 1; }
[[ "$CMS_ORIGIN" == "https://admin-dev.zetruv.com" ]] || { echo "Unexpected DEV CMS_ORIGIN: $CMS_ORIGIN" >&2; exit 1; }
[[ "$CMS_ORIGIN_LEGACY" == "https://admin.zetruv.dualangka.com" ]] || { echo "Unexpected DEV CMS_ORIGIN_LEGACY: $CMS_ORIGIN_LEGACY" >&2; exit 1; }
...
echo '1/5 CORS from new DEV admin'
assert_cors "$CMS_ORIGIN"

echo '2/5 protected CMS rejects unauthenticated request'
...
echo '4/5 JWT opens protected CMS endpoint'
...
echo '5/5 legacy admin CORS remains valid during cutover'
assert_cors "$CMS_ORIGIN_LEGACY"

echo 'PASS: DEV admin backend handoff is healthy; new admin origin, login, JWT authorization, protected CMS API, and legacy admin origin all pass.'
