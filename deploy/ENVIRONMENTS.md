...
DEV/STAGING may use mock providers for payment, game-account validation, shipping, and AUTO_ID fulfillment. AUTO_ID provider selection is driven by the active provider game/SKU mappings in the database; there is no global AUTO_ID provider environment switch. `ASPNETCORE_ENVIRONMENT=Production` remains reserved for the future client production environment: mock adapters are not registered, startup rejects active AUTO_ID mappings that still target `mock`, and real provider configuration must be supplied.
